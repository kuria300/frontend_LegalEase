...
export default function LawyerDashboardLayout() {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("legalease_lawyer_id");
    navigate("/login");
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#f0f2f5]">
      ...
      `}>
        ...
        {/* Profile block */}
        <div className="flex items-center gap-2 px-4 py-3 border-b border-[#e4e7ec] min-h-[63px]">
          {loading ? (
            <div className="flex items-center justify-center w-full py-1">
              <Loader2 className="w-5 h-5 text-[#000f26] animate-spin" />
            </div>
          ) : (
            <>
              <div className="w-[38px] h-[38px] rounded-full border-2 border-[#000f26] p-[2px] shrink-0">
                <div className="w-full h-full rounded-full bg-[#000f26] flex items-center justify-center text-white text-xs font-bold">
                  {initials}
                </div>
              </div>
              <div className="overflow-hidden">
                <p className="text-[0.8rem] font-semibold text-[#101828] truncate">
                  {user?.name || "Lawyer Profile"}
                </p>
          ...
          {TOP_NAV.map(({ to, icon: Icon, label, end }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                ${isActive
                  ? "bg-[#f5f8ff] text-[#000f26] font-semibold border-l-[3px] border-[#000f26]"
                  : "text-[#344054] hover:bg-[#f5f8ff] hover:text-[#000f26]"
                }`
              }
            >
              <Icon size={18} />
          ...
          {BOTTOM_NAV.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-[10px] px-[10px] py-[9px] rounded-md text-[0.83rem] font-medium no-underline transition-colors
                ${isActive
                  ? "bg-[#f5f8ff] text-[#000f26] font-semibold"
                  : "text-[#344054] hover:bg-[#f5f8ff] hover:text-[#000f26]"
                }`
              }
            >
